...
static void scrob_md5_transform(uint32_t state[4], const uint8_t block[64]);
...
void scrob_md5_update(scrob_md5_ctx *context, const uint8_t *input, size_t input_len) {
  ...
  uint32_t index;
  uint32_t part_len;
  const uint32_t input_len_32 = (uint32_t)input_len;

  /* Compute number of bytes mod 64 */
  index = (context->count[0] >> 3U) & 0x3FU;

  /* Update number of bits */
  if ((context->count[0] += (input_len_32 << 3U)) < (input_len_32 << 3U)) {
    context->count[1]++;
  }
  context->count[1] += (input_len_32 >> 29U);
...
*/
  if (input_len >= part_len) {
    ...
    scrob_md5_transform(context->state, context->buffer);

    for (i = part_len; i + 63U < input_len; i += 64U) {
      scrob_md5_transform(context->state, &input[i]);
    }
...
*/
  ...
  scrob_md5_update(context, PADDING, pad_len);

  /* Append length (before padding) */
  scrob_md5_update(context, bits, 8U);
...
static void encode(uint8_t *output, const uint32_t *input, size_t len) {
 ...
 output[j+1] = (uint8_t)((input[i] >> 8U) & 0xffU);
 output[j+2] = (uint8_t)((input[i] >> 16U) & 0xffU);
 output[j+3] = (uint8_t)((input[i] >> 24U) & 0xffU);
  }
}
